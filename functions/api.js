/**
 * Fair Ride - Netlify Serverless Function
 * Backend using Supabase REST API via native fetch
 * Eliminates all TCP cold-start timeout issues completely.
 */
...
// ─── Health Check ─────────────────────────────────────────────────────────────
router.get('/', (req, res) => {
  res.json({ status: '✅ Fair Ride API running', backend: 'Supabase REST API', timestamp: new Date().toISOString() });
});
...
router.patch('/rides/:id/complete', async (req, res) => {
  try {
    ...
    const r = data[0] || req.body; // PATCH doesn't always return row unless Representation is used
